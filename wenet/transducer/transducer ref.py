...
class Transducer(ASRModel):
    ...
    def __init__(
        ...
        self.cache = self.predictor.init_state(1,
            method="zero",
            device=torch.device('cpu'))
        self.pred_out_step = torch.zeros([1,1,self.predictor.output_size])
        self.pred_input_step = torch.tensor([self.blank]).reshape(1, 1)
        self.per_frame_noblk = 0
        self.prev_out_nblk = True
        encoder_dim = encoder._output_size
        decoder_dim = predictor.output_size 
        self.simple_lm_proj = ScaledLinear(
	    encoder_dim, vocab_size, initial_speed=0.5
        ) 
        self.simple_am_proj = ScaledLinear(decoder_dim, vocab_size)
    ...
    def forward(
        ...
        loss = rnnt_loss_simple(lm=lm,
                         ...
                         reduction='mean',
                         #rnnt_type='modified',
                         #delay_penalty=0.0030
                        )
        # rnnt_text = rnnt_text.to(torch.int32)
        # loss_src = torchaudio.functional.rnnt_loss(joint_out,
        #                                        rnnt_text,
        #                                        encoder_out_lens,
        #                                        rnnt_text_lengths,
        #                                        blank=self.blank,
        ...
        return {
            ...
            'loss_ctc': loss_ctc,
            'loss_rnnt': loss_rnnt,
            'time_forward': forward_rnnt,
            'time_loss': forward_loss
        }

    def net_input_process(
        self,
        key,
        feats,
        target,
        ...
        return {
            "speech": feats.to(device),
            "text": target.to(device),
            "speech_lengths": feats_lengths.to(device),
            "text_lengths": target_lengths.to(device),
    ...
    def forward_greedy_search(
        ...
        t = 0
        hyps = [0 for i in range(encoder_out.shape[1])]
        hyps_index = 0
        # prev_out_nblk = True
        # pred_out_step = None
        per_frame_max_noblk = n_steps
        # per_frame_noblk = 0
        # pred_out_step = torch.zeros([1,1,self.predictor.output_size])
        new_cache = self.cache
        
        while t < encoder_out_lens:
            encoder_out_step = encoder_out[:, t:t + 1, :]  # [1, 1, E]
            
            if self.prev_out_nblk:
                step_outs = self.predictor.forward_step(self.pred_input_step, padding,
                                                        self.cache)  # [1, 1, P]
                self.pred_out_step, new_cache = step_outs[0], step_outs[1]
            # else:
            #     self.pred_out_step, new_cache = self.pred_out_step, new_cache

            joint_out_step = self.joint(encoder_out_step,
                                        self.pred_out_step)  # [1,1,v]
            ...
            if joint_out_max != self.blank:
                hyps[hyps_index]=joint_out_max.item()
                hyps_index+=1
                self.prev_out_nblk = True
                self.per_frame_noblk = self.per_frame_noblk + 1
                self.pred_input_step = joint_out_max.reshape(1, 1)
                # state_m, state_c =  clstate_out_m, state_out_c
                self.cache = new_cache

            if joint_out_max == self.blank or self.per_frame_noblk >= per_frame_max_noblk:
                if joint_out_max == self.blank:
                    self.prev_out_nblk = False

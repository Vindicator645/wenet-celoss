...
class ContextBias(torch.nn.Module):
    def __init__(
        ...
        if self.encoder_type == 'mytransformer':
            ...
            #tyxu
            self.predictor_bias = MultiHeadedAttentionPure(
                n_head=self.attention_heads,
                n_feat=self.embedding_size,
                dropout_rate=0.0,
                chunk_size=-1,
            ...
            #tyxu
            self.predictor_bias = MultiHeadedAttention(
                n_head=self.attention_heads,
                n_feat=self.embedding_size,
                dropout_rate=0.0
            )
        ...
        # kxhuang 
        self.encoder_bias_combine = torch.nn.Linear(self.embedding_size * 2, self.embedding_size)
        self.encdoer_bias_bias_norm = torch.nn.LayerNorm(self.embedding_size)
        self.encdoer_bias_out_norm = torch.nn.LayerNorm(self.embedding_size)
        #tyxu
    ...
    def forward_predictor_bias(self, bias_hidden, predictor_out):
        ...
        predictor_out_bias, _ = self.predictor_bias(predictor_out, bias_hidden, bias_hidden)

...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  for ((i = 0; i < $num_gpus; ++i)); do
  {
    ...
    python wenet_cascade_bias/bin/train.py --gpu $gpu_id \
      --config $train_config \
      --data_type raw \
      --symbol_table $dict \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  if [ ${average_checkpoint} == true ]; then
    ...
    python wenet_cascade_bias/bin/average_model.py \
    ...
    {
      {
        ...
        python wenet_cascade_bias/bin/recognize.py --gpu $gpu_id \
          --mode $mode \
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  python wenet_cascade_bias/bin/export_jit.py \
    --config $dir/train.yaml \

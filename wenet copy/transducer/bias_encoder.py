...
class ContextBiasEncoder(torch.nn.Module):
    ...
    def forward_context_enc_mix(self, hw_list_phone, hw_list_char):
        context_bias_vector_phone = self.context_extracter(hw_list_phone)
        h_context_phone = self.context_enc(context_bias_vector_phone.unsqueeze(0), [context_bias_vector_phone.shape[0]])

        context_bias_vector_char = self.mix_context_extracter(hw_list_char)
        h_context_char = self.mix_context_enc(context_bias_vector_char.unsqueeze(0), [context_bias_vector_char.shape[0]])

        h_context = torch.cat([h_context_phone, h_context_char], axis = -1)
        h_context = self.mix_linear(h_context)
        return h_context
    ...
    def forward_enc_split(self, h_enc, h_context, utt_for_each, device):
        h_enc_proj = self.enc_proj(h_enc)
        h_enc_bias = torch.zeros(h_enc_proj.shape).to(device)
        begin_point = 0
        for i in range(utt_for_each.shape[0]):
            h_enc_proj_split = h_enc_proj[i,:,:].unsqueeze(0)
            #print(begin_point, begin_point + utt_for_each[i], h_context.shape)
            h_context_split = h_context[0, begin_point : begin_point + utt_for_each[i] ,:]
            begin_point += utt_for_each[i]
            #print(h_enc_proj_split.shape, h_context_split.shape)
            h_enc_bias_split = self.encoder_bias(h_enc_proj_split, h_context_split, h_context_split)
            h_enc_bias[i,:,:] = h_enc_bias_split
        h_enc_hw_output = self.hw_output_layer_enc(h_enc_bias)
        if(self.bias_mix_type == "concat"):
            h_enc_cat = torch.cat([self.encoder_norm(h_enc_proj), self.encoder_bias_norm(h_enc_bias)], axis = -1)
            h_enc_out = self.encoder_output_layer(h_enc_cat) + h_enc
    ...
    def forward_dec_split(self, h_dec, h_context, utt_for_each, device):
        h_dec_proj = self.dec_proj(h_dec)
        h_dec_bias = torch.zeros(h_dec_proj.shape).to(device)
        begin_point = 0
        for i in range(h_dec.shape[0]):
            h_dec_proj_split = h_dec_proj[i,:,:].unsqueeze(0)
            h_context_split = h_context[0, begin_point : begin_point + utt_for_each[i] ,:]
            begin_point += utt_for_each[i]
            h_dec_bias_split = self.decoder_bias(h_dec_proj_split, h_context_split, h_context_split)
            h_dec_bias[i,:,:] = h_dec_bias_split
        h_dec_hw_output = self.hw_output_layer_dec(h_dec_bias)
        if(self.bias_mix_type == "concat"):
            h_dec_cat = torch.cat([self.decoder_norm(h_dec_proj), self.decoder_bias_norm(h_dec_bias)], axis = -1)
            h_dec_out = self.decoder_output_layer(h_dec_cat) + h_dec

...
from wenet.transducer.predictor_char import PredictorBase
...
class Transducer(ASRModel):
    """Transducer-ctc-attention hybrid Encoder-predictor_char-Decoder model"""
    ...
    def __init__(
        ...
        blank: int,
        encoder: nn.Module,
        predictor_char: PredictorBase,
        predictor_phone: PredictorBase,
        joint_char: nn.Module,
        joint_phone: nn.Module,
        attention_decoder: Optional[Union[TransformerDecoder,
                                          BiTransformerDecoder]] = None,
        ctc: Optional[CTC] = None,
        non_causal_encoder: nn.Module=None,
        ...
        streaming_transducer_weight: float = 0.3,
        ...
        self.predictor_char = predictor_char
        self.predictor_phone = predictor_phone
        self.joint_char = joint_char
        self.joint_phone = joint_phone
        self.bs = None
        # Note(Mddct): decoder also means predictor_char in transducer,
    ...
    def forward(
        ...
        """Frontend + Encoder + predictor_char + joint_char + loss
        ...
        encoder_out_lens_s = encoder_mask.squeeze(1).sum(1)
        #cascade Encoder
        encoder_out_s = encoder_out.clone()
        encoder_out, encoder_mask = self.non_causal_encoder(encoder_out, torch.tensor([encoder_out.shape[1]],device=speech.device,dtype=torch.int))
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
        
        #predictor_phone
        phone_in_pad = add_blank(phone, self.blank, self.ignore_id)
        predictor_phone_out=self.predictor_phone(phone_in_pad)
        # kxhuang
        # get encoder_out_bias
        # if bias_hidden != 0:
        encoder_out = self.context_bias.forward_encoder_bias(bias_hidden, encoder_out)
        # predictor_char
        text_in_pad = add_blank(text, self.blank, self.ignore_id)
        predictor_char_out = self.predictor_char(text_in_pad)
        joint_phone_out = self.joint_phone(encoder_out_s, predictor_phone_out)
        # kxhuang 
        # get predictor_out_bias
        # if bias_hidden !=0:
        predictor_char_out = self.context_bias.forward_predictor_bias(bias_hidden, predictor_char_out)
        
        # joint_char
        joint_char_out = self.joint_char(encoder_out, predictor_char_out)
        # NOTE(Mddct): some loss implementation require pad valid is zero
        # torch.int32 rnnt_loss required
        rnnt_phone = phone.to(torch.int64)
        rnnt_phone = torch.where(rnnt_phone == self.ignore_id, 0,
                                rnnt_phone).to(torch.int32)
        rnnt_phone_lengths = phone_lengths.to(torch.int32)
        encoder_out_lens_s = encoder_out_lens_s.to(torch.int32)
        # print(joint_char_out.shape)
        # print(rnnt_text.shape)
        # print(encoder_out_lens.shape)
        # print(rnnt_text_lengths.shape)
        streaming_loss= torchaudio.functional.rnnt_loss(joint_phone_out,
                                               rnnt_phone,
                                               encoder_out_lens_s,
                                               rnnt_phone_lengths,
                                               blank=self.blank,
        ...
        loss = torchaudio.functional.rnnt_loss(joint_char_out,
                                               rnnt_text,
                                               encoder_out_lens,
                                               rnnt_text_lengths,
                                               blank=self.blank,
        ...
        loss_rnnt = loss + self.streaming_transducer_weight*streaming_loss
    ...
    def init_bs(self):
        if self.bs is None:
            self.bs = PrefixBeamSearch(self.encoder, self.predictor_char,
                                       self.joint_char, self.ctc, self.blank)
    ...
    def _cal_transducer_score(
        ...
        # 1. Forward predictor_char
        predictor_char_out = self.predictor_char(hyps_pad_blank)
        # 2. Forward joint_char
        joint_char_out = self.joint_char(encoder_out, predictor_char_out)
        rnnt_text = hyps_pad.to(torch.int64)
        rnnt_text = torch.where(rnnt_text == self.ignore_id, 0,
                                rnnt_text).to(torch.int32)
        # 3. Compute transducer loss
        loss_td = torchaudio.functional.rnnt_loss(joint_char_out,
                                                  rnnt_text,
                                                  xs_in_lens,
                                                  hyps_lens.int(),
                                                  blank=self.blank,
    ...
    def forward_predictor_step(
        ...
        return self.predictor_char.forward_step(xs, padding, cache)

    @torch.jit.export
    def forward_joint_step(self, enc_out: torch.Tensor,
                           pred_out: torch.Tensor) -> torch.Tensor:
        return self.joint_char(enc_out, pred_out)

    @torch.jit.export
    def forward_predictor_init_state(self) -> List[torch.Tensor]:
        return self.predictor_char.init_state(1, device=torch.device("cpu"))

...
from wenet.transducer.bias_encoder import ContextBiasEncoder
...
class Transducer(ASRModel):
    ...
    def __init__(
        ...
        self.joint = joint
        self.bs = None
        self.context_bias_encoder = ContextBiasEncoder(args)
    ...
    def forward(
        ...
        **input_dict,
        ...
        #读取热词列表                                 
        hw_list = input_dict['input-context']
        # 获取热词隐向量
        h_context = self.context_bias_encoder.forward_context_enc(hw_list)
        ...
        # Non-streaming transducer encoder
        encoder_out_s = encoder_out.clone()
        encoder_out,  = self.noncasual_enc(encoder_out_s,speech_length)
        encoder_out,hw_output_enc=self.context_bias_encoder.forward_enc(encoder_out,h_context)
        # predictor
        ys_in_pad = add_blank(text, self.blank, self.ignore_id)
        predictor_out = self.predictor(ys_in_pad)
        predictor_out_ns, hw_output_dec = self.context_bias_encoder.forward_dec(predictor_out, h_context)
